defmodule GameTest do
  ...
  test "init creates the game as a GenServer" do
    ...
    {:ok, game_pid} = Wordiverse.Game.start_link(type, player_1_id, player_2_id)
    game = Wordiverse.Game.get(game_pid, :full)
  ...
  test "get gets the game various states :full is special and gets all things" do
    ...
    {:ok, game_pid} = Wordiverse.Game.start_link(type, player_1_id, player_2_id)
    game = Wordiverse.Game.get(game_pid, :full)
    assert Wordiverse.Game.get(game_pid, :board) == game.board
    assert Wordiverse.Game.get(game_pid, :player_1) == game.player_1
    assert Wordiverse.Game.get(game_pid, :player_2) == game.player_2
    assert Wordiverse.Game.get(game_pid, :tiles) == game.tiles_in_pile
  end
end
